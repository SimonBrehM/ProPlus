"""
This module provides functions for creating Bokeh plots.
"""
import math
...
def moyenne_graph(current:float, last:float, suggestive:bool=False):
    """
    Generates a graph from the last and current moyenne, with suggestive style possible
    """
    ...
    data = {
        'stable_start': [stable_start_angle],
        'stable_end': [stable_end_angle],
        'difference_start': [difference_start_angle],
        'difference_end': [difference_end_angle]
    }

    #Render
    source = ColumnDataSource(data=data)

    p.wedge(x=0, y=0, start_angle='stable_start', end_angle='stable_end',
            radius=0.8, color="#ff5b5b", source=source)

    if difference!=0:
        p.wedge(x=0, y=0, start_angle='difference_start', end_angle='difference_end',
                radius=0.8, color=difference_color, source=source)

    p.wedge(x=0, y=0,
            start_angle=math.pi/2,
            end_angle=math.pi/2 - 1/360 * 2 * math.pi,
            radius=0.6, color="#FAF9F9", source=source)
